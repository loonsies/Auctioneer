...
function auctionHouse.clearSales()
    ...
    for slot = 0, 6 do
        if (auctioneer.auction_box[slot] ~= nil) and (auctioneer.auction_box[slot].status == "Sold" or auctioneer.auction_box[slot].status == "Not Sold") then
...
function auctionHouse.proposal(bid, name, vol, price)
    ...
    if (utils.hasFlag(item.Flags, itemFlags["NoAuction"]) == true) then
        print(
            chat.header(addon.name):append(
                chat.message(string.format("AH Error: %s is not purchasable via the auction house.", item.Name[1]))
            )
        )
        return false
    end
