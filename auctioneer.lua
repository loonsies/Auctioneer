    ...
    for x = 0, 6 do
        if (auction_box[x] ~= nil) then
            ...
            if (auction_box[x].status ~= "Empty") then
                local timer =
                    auction_box[x].status == "On auction" and auction_box[x].timestamp + 829440 or
                    auction_box[x].timestamp
                if (auctioneer.settings.auction_list.timer) then
                    str =
                        str ..
                        string.format(
                            " %s",
                            (auction_box[x].status == "On auction" and os.time() - timer > 0) and "Expired" or
                                timef(math.abs(os.time() - timer))
                        )
                end
...
ashita.events.register(
    ...
    function(cmd, nType)
        ...
        if
            (args[1] ~= "/ah" and args[1] ~= "/buy" and args[1] ~= "/sell" and args[1] ~= "/inbox" and
                args[1] ~= "/outbox" and
                args[1] ~= "/ibox" and
                args[1] ~= "/obox")
         then
    ...
    function(e)
        if (e.id == 0x04C) then
            ...
            if (pType == 0x04) then
                ...
                if
                    (last4E ~= nil and e.data:byte(7) == 0x01 and slot ~= nil and last4E ~= nil and
            ...
            elseif (pType == 0x0A) then
                if (e.data:byte(7) == 0x01) then
                    ...
                    if (auction_box[e.data:byte(6)] == nil) then
                        auction_box[e.data:byte(6)] = {}
                    end
...
function ah_proposal(bid, name, vol, price)
    ...
    if (item == nil) then
        print(
            chat.header(addon.name):append(chat.message(string.format('AH Error: "%s" not a valid item name.', name)))
        )
        return false
    end
    ...
    if (has_flag(item.Flags, ItemFlags["NoAuction"]) == true) then
        print(
            chat.header(addon.name):append(
                chat.message(string.format("AH Error: %s is not purchasable via the auction house.", item.Name[0]))
            )
        )
        return false
    end
    ...
    price = price:gsub("%p", "")
    if
        (price == nil) or (string.match(price, "%a") ~= nil) or (tonumber(price) == nil) or (tonumber(price) < 1) or
            (bid == "/sell" and tonumber(price) > 999999999) or
            (bid == "/buy" and tonumber(price) > AshitaCore:GetMemoryManager():GetInventory():GetContainerItem(0, 0).Count)
     then
    ...
    elseif (bid == "/sell") then
        if (auction_box == nil) then
            print(
                chat.header(addon.name):append(
                    chat.message("AH Error: Click auction counter or use /ah to initialize sales.")
                )
            )
            return false
        end
        ...
        if (index == nil) then
            print(
                chat.header(addon.name):append(
                    chat.message(
                        string.format(
                            "AH Error: %s of %s not found in inventory.",
                            single == 1 and "Single" or "Stack",
                            item.Name
                        )
                    )
